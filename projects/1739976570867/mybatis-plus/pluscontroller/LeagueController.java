package com.hzy.controller;
import com.alibaba.fastjson.JSON;
import com.hzy.entity.League;
import com.hzy.mapper.LeagueMapper;
import com.hzy.util.ReturnT;
import lombok.extern.slf4j.Slf4j;
import org.apache.commons.lang3.StringUtils;
import org.springframework.beans.factory.annotation.Autowired;
import org.springframework.web.bind.annotation.*;
import com.baomidou.mybatisplus.core.conditions.query.QueryWrapper;
import com.baomidou.mybatisplus.core.metadata.IPage;
import com.baomidou.mybatisplus.extension.plugins.pagination.Page;
import org.springframework.web.servlet.ModelAndView;

import java.util.Date;
import java.util.List;
import java.util.Map;
/**
* @description league控制器
* @author capkin
* @date 2025-02-19
*/
@Slf4j
@RestController
@RequestMapping("/league")
public class LeagueController {

    @Autowired
    private LeagueMapper leagueMapper;

    /**
    * 新增或编辑
    */
    @PostMapping("/save")
    public Object save(@RequestBody League league){
        log.info("league:"+JSON.toJSONString(league));
        League oldLeague = leagueMapper.selectOne(new QueryWrapper<League>().eq("league_id",league.getLeagueId()));
        league.setUpdateTime(new Date());
        if(oldLeague!=null){
            leagueMapper.updateById(league);
        }else{
        if(leagueMapper.selectOne(new QueryWrapper<League>().eq("league_name",league.getLeagueName()))!=null){
            return ReturnT.error("保存失败，名字重复");
        }
        league.setCreateTime(new Date());
        leagueMapper.insert(league);
        }
        return ReturnT.success("保存成功");
    }

    /**
    * 删除
    */
    @PostMapping("/delete")
    public Object delete(int id){
    League league = leagueMapper.selectOne(new QueryWrapper<League>().eq("league_id",id));
        if(league!=null){
            leagueMapper.deleteById(id);
            return ReturnT.success("删除成功");
        }else{
            return ReturnT.error("没有找到该对象");
        }
    }

    /**
    * 查询
    */
    @PostMapping("/find")
    public Object find(int id){
    League league = leagueMapper.selectOne(new QueryWrapper<League>().eq("league_id",id));
        if(league!=null){
            return ReturnT.success(league);
        }else{
            return ReturnT.error("没有找到该对象");
        }
    }

    /**
    * 自动分页查询
    */
    @PostMapping("/list")
    public Object list(String searchParams,
    @RequestParam(required = false, defaultValue = "0") int page,
    @RequestParam(required = false, defaultValue = "10") int limit) {
        log.info("page:"+page+"-limit:"+limit+"-json:"+ JSON.toJSONString(searchParams));
        //分页构造器
        Page<League> buildPage = new Page<League>(page,limit);
        //条件构造器
        QueryWrapper<League> queryWrapper = new QueryWrapper<League>();
        if(StringUtils.isNotEmpty(searchParams)&&JSON.isValid(searchParams)) {
            League league = JSON.parseObject(searchParams, League.class);
            queryWrapper.eq(StringUtils.isNoneEmpty(league.getLeagueName()), "league_name", league.getLeagueName());
        }
        //执行分页
        IPage<League> pageList = leagueMapper.selectPage(buildPage, queryWrapper);
        //返回结果
        return .PAGE(pageList.getRecords(),pageList.getTotal());
    }
    /**
    * 手工分页查询(按需使用)
    */
    /*@PostMapping("/list2")
    public Object list2(String searchParams,
    @RequestParam(required = false, defaultValue = "0") int page,
    @RequestParam(required = false, defaultValue = "10") int limit) {
        log.info("searchParams:"+ JSON.toJSONString(searchParams));
        //通用模式
        League queryParamDTO = JSON.parseObject(searchParams, League.class);
        //专用DTO模式
        //QueryParamDTO queryParamDTO = JSON.parseObject(searchParams, QueryParamDTO.class);
        //queryParamDTO.setPage((page - 1)* limit);
        //queryParamDTO.setLimit(limit);
        //(page - 1) * limit, limit
        List<League> itemList = leagueMapper.pageAll(queryParamDTO,(page - 1)* limit,limit);
        Integer itemCount = leagueMapper.countAll(queryParamDTO);
        //返回结果
        return ReturnT.success.PAGE(itemList,itemCount);
    }*/
    @GetMapping("/list")
    public ModelAndView listPage(){
        return new ModelAndView("league-list");
    }

    @GetMapping("/edit")
    public ModelAndView editPage(int id){
        League league = leagueMapper.selectOne(new QueryWrapper<League>().eq("league_id",id));
        return new ModelAndView("league-edit","league",league);
    }

    /**
    * 发布/暂停(如不需要请屏蔽)
    */
    @PostMapping("/publish")
    public Object publish(int id,Integer status){
        League league = leagueMapper.selectOne(new QueryWrapper<League>().eq("league_id",id));
        if(league!=null){
            league.setUpdateTime(new Date());
            league.setStatus(status);
            leagueMapper.updateById(league);
            return ReturnT.success((status==1)?"已发布":"已暂停");
        }else if(status.equals(league.getStatus())){
            return ReturnT.error("状态不正确");
        }else{
            return ReturnT.error();
        }
    }

    /**
    * 执行(如不需要请屏蔽)
    */
    @PostMapping("/execute")
    public Object execute(){
        return ReturnT.success;
    }
}
}



